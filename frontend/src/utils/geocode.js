...
export function getTrafficCondition() {
  ...
  if ((h >= 8 && h < 11) || (h >= 17 && h < 21))
    return { multiplier: 1.00, label: "Heavy Traffic", color: "#ef4444", icon: "🔴" };
  if (h >= 11 && h < 17)
    return { multiplier: 1.00, label: "Moderate Traffic", color: "#f59e0b", icon: "🟡" };
  if (h >= 21 || h < 6)
    return { multiplier: 0.95, label: "Light Traffic", color: "#22c55e", icon: "🟢" };
